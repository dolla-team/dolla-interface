...
export default function useGenerateKey() {
    const [keyPairSigner, setKeyPairSigner] = useState<KeyPairSigner | null>(null);
    const { setPublicKey, setPrivateKey, publicKey, privateKey } = useNearKeyStore();
    const { signMessage } = useSignMessage();

    async function generateKeyPair() {
        if (publicKey && privateKey) {
            const newKeyPairSigner = KeyPairSigner.fromSecretKey(('ed25519:' + privateKey) as any);
        ...
        const { publicKey: shortPublicKey, keyPairSigner: newKeyPairSigner, privateKey: newPrivateKey } = createKeyPair();
    ...
    function saveKeyPair(publicKey: string, keyPairSigner: KeyPairSigner, privateKey: string) {
