...
export default function History({
  ...
  return (
    <div className={clsx('pt-[0px] text-[#2B3337]', isMobile ? 'p-0' : 'p-[18px]')}>
      ...
      <div
        ref={containerRef}
        className={clsx('overflow-y-auto', isMobile ? 'max-h-[70vh] min-h-[400px]' : 'h-[350px]')}
      >
        {loading && (
            ...
            {data.map((record: any) => (
              ...
              >
                {COLUMNS.map(column => (
                  ...
                  >
                    {column.key === 'date' &&
                      dayjs(record.created_time).format('HH:mm D MMM, YYYY')}
                    {column.key === 'volume' && `${record.volume} ${itemsMap[record.token]?.name}`}
                    {['number', 'reward'].includes(column.key) && record[column.key]}
                  </div>
                ))}
              </div>
            ))}
        ...
        {data.length === 0 && !loading && (
          <Empty text="No data" className="h-[300px] flex items-center justify-center" />
        )}
        {data.length > 0 && (
