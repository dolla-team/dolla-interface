...
export default function useProvably({ currentPool }: { currentPool: any }) {
      ...
      if (provablyData.data?.data?.list?.length > 0) {
        const list = provablyData.data?.data?.list.map((item: any) => {
          ...
          return {
            ...
            marketSize: item.reward_token_info
              ? item.reward_amount / 10 ** item.reward_token_info[0]?.decimals +
                " " +
                item.reward_token_info[0]?.symbol
              : 0,
            winNo: winNumber ? `0-${winNumber}` : "",
            settleTX: winNumber
              ? formatAddress(item.winner_info?.verify_result?.settle_tx_hash)
              : "",
  ...
  const verifySolana = useCallback(async (value: string) => {
    ...
    try {
      ...
      const signedData = await axiosInstance.post(`/api/v1/verify/sol`, params);
