...
export default function VerifyList({
  ...
  list: any[];
  loading: boolean;
  setYouParticipateOnly: (value: boolean) => void;
  youParticipateOnly: boolean;
  offset: number;
    ...
    <div>
      <div className="mt-[20px] h-[444px] overflow-y-auto text-white pb-[20px]">
        <div className="flex items-center justify-between px-[10px]">
          ...
          >
            <div className="border border-[#743EFF] rounded-full w-[15px] h-[15px] flex items-center justify-center mr-2">
              {youParticipateOnly && (
                <div className="w-[9px] h-[9px] bg-[#743EFF] rounded-full"></div>
              )}
            </div>
        ...
        <div
          ...
          className="w-full overflow-x-scroll pl-[10px] relative mt-[10px]"
        >
          <div className={clsx("", isMobile ? "w-[800px]" : "w-full ")}>
            <table className="sticky-table w-full">
              <thead className="">
                <tr className="text-[14px] relative text-[#8A87AA] sticky-column">
                  {COLUMNS.map((column, idx) => (
                    <th
                      ...
                      className={clsx(
                        ...
                        column.isFloat && isScroll
                          ? "shadow-[2px_2px_8px_0px_rgba(0,0,0,0.2)] bg-[#35302B]"
                          : ""
                      )}
                      key={column.key}
                    >
                      {column.label}
                      {idx === 0 && isScroll && (
                        <div className="absolute h-full w-[15px] z-1 top-0 bg-[#35302B] left-[-15px]"></div>
                      )}
                    </th>
                  ))}
                </tr>
                ...
                {!loading &&
                  ...
                  list?.map((record: any) => (
                    <>
                      <tr
                        key={record.id}
                        className={clsx("relative h-[50px]  text-[16px]")}
                      >
                        {COLUMNS.map((column, idx) => (
                          <td
                            key={column.key}
                          ...
                          >
                            ...
                            {idx === 0 && isScroll && (
                              <div className="absolute h-full w-[20px] z-1 top-0 bg-[#35302B] left-[-12px]"></div>
                            )}
                          </td>
                        ))}
                      </tr>
