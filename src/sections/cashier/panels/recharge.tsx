  ...
  return (
    <>
      <div className={clsx("flex flex-col items-center", className)}>
        ...
        <div
          className={clsx(
            "mt-[24px] p-[12px] rounded-[10px] bg-white w-full",
            tokenPanelClassName
          )}
        >
          <div className="flex items-center gap-[6px]">
            <span className="text-[18px] font-[700]">{token.symbol} </span>
            <img src={token.chainLogo} className="w-[16px] h-[16px]" />
            <div className="text-[12px]">{token.chainName}</div>
          </div>
          <div className="mt-[20px] text-[12px] break-all font-[300]">
            {!loading && tokenAddress}
          </div>
          <Button
            className="w-full h-[50px] mt-[6px] flex items-center gap-[10px] !bg-[#000] !text-white"
            onClick={() => {
