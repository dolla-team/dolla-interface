...
const TOKENS = [
    {
        ...
        address: "nep141:17208628f84f5d6ad33f0da3bbbeb27ffcb398eac501a31bd6ad2011e36133a1",
        decimals: 6,
    },
    {
        symbol: "BTC",
...
export default function TokenSelector({
    ...
    const usedChains = useMemo(() => {
        return tokens.filter((token: any) => token.symbol.toUpperCase().includes(selectedToken.symbol.toUpperCase())
    ...
    useEffect(() => {
        if (defaultToken) {
            setSelectedToken(TOKENS.find((token) => token.symbol.toUpperCase() === defaultToken.toUpperCase()));
        }
        if (defaultValue) {
    ...
    useEffect(() => {
        ...
        if (debouncedAmount && config
            && selectedChain && user?.wallet?.address && selectedToken) {
            (async () => {
                try {
                    setQuote(null);
                    setLoading(true);
                    const qoute = await generateDepositAddress({
                        ...
                        amount: new Big(debouncedAmount).mul(10 ** selectedToken.decimals).toString(),
    ...
    return (
        <div className="relative h-[calc(100vh-100px)]" onClick={(e) => {
            setIsOpen(false);
        }}>
            <div className="text-[16px] text-center text-black mb-[20px]">Input Deposit Amount</div>
                    ...
                    >
                        <div className="relative">
                            <div className="w-[32px] h-[32px] flex items-center justify-center">
                                <img className="w-full h-full object-cover" src={selectedToken?.icon} />
                            </div>
                        ...
                        >
                            {TOKENS.map((token) => (
                                ...
                                >
                                    <div className="relative">
                                        <div className="w-[32px] h-[32px] flex items-center justify-center">
                                            <img className="w-full h-full object-cover" src={token.icon} />
                                        </div>
                                        {/* <div className="absolute -bottom-1 -right-1 w-[16px] h-[16px] flex items-center justify-center">
            ...
            <div className="mt-[10px] text-[12px] leading-[18px] text-[#8A87AA]">
                The third-party bridge service will be used during the recharge process, which requires at least an additional recharge of <span className="font-bold text-black">{defaultValue || '-'}</span>
            </div>

            <div className="mt-[24px]">
                <div className="text-[16px] font-[500] text-center mb-[16px]">Select Receiving Network</div>
                <div className="flex flex-col gap-[8px] max-h-[400px] overflow-y-auto">
                    {usedChains.map((network) => (
                        ...
                        >
                            <div className="w-[32px] h-[32px] flex items-center justify-center">
                                <img src={chainConfig[network?.blockchain]?.icon} alt={chainConfig[network?.blockchain]?.name || network.blockchain} className="w-full h-full object-cover rounded-[6px]" />
                            </div>
                            <div className="text-[16px] font-[400] text-black">{chainConfig[network?.blockchain]?.name || network.blockchain}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0">
                {
                    quote && quote.amountOut && Number(quote.amountOut) < Number(defaultValue) && <div className="bg-[#FFE5E5] rounded-[12px] px-[16px] py-[12px] text-center pb-[24px] mb-[-10px]">
                        <div className="text-[#FF3D2F] text-[12px] font-[400] leading-[120%]">
                            You may only receive <span className="font-bold">-{quote ? quote.amountOut : '-'} {selectedToken?.symbol || '-'}</span>, which is not enough to create a minimal market.
                        </div>
                    </div>
                }
                ...
                <button
                    ...
                    className="w-full bg-black cursor-pointer text-white text-[14px] font-[400] rounded-[12px] py-[20px] transition-colors duration-200 hover:bg-[#222] disabled:opacity-50 disabled:cursor-not-allowed"
                    onClick={() => {
                        if (quote || loading) {
                            onConfirm();
                        }
