...
export default function TokenSelector({
  ...
  useEffect(() => {
    if (
      debouncedAmount &&
      config &&
      Number(debouncedAmount) < Number(config.min_deposit_amount / 10 ** 6)
    ) {
      fail({
        title: `Minimum deposit amount is ${
          config.min_deposit_amount / 10 ** 6
        } USDC`
      });
      return;
    }
