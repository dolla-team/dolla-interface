...
export default function Withdraw() {
  ...
  const isAddressValid = useMemo(() => {
    return (
      receiveAddress.length === 42 && receiveAddress.startsWith('0x') && receiveAddress === address
    )
  }, [receiveAddress])
  ...
  return (
    <div className="pt-[20px]">
      ...
      {selectedItem?.type === 'coin' && (
        <>
          <div className="text-[14px] text-[#ADBCCF] mt-[20px]">Withdraw Amount</div>
          <div className="flex items-center mt-[6px] gap-[12px]">
            ...
            <div className="flex items-center text-[14px] text-white">
              <span>{formatNumber(tokenBalance, 0, true, { isShort: true })}</span>
              <img
                src="/tokens/usdc.png"
                alt="bid-coins"
      ...
      <>
        <div className="text-[14px] text-[#ADBCCF] mt-[20px]">Receive Address</div>
        <input
          className={clsx(
...
const Item = ({ data, onClick, active }: { data: any; onClick: () => void; active: boolean }) => {
  return (
    <div
      onClick={onClick}
      className={clsx(
    ...
    >
      ...
      {data.amount ? (
        <div className="text-white">{formatNumber(data.amount, 0, true, { isShort: true })}</div>
      ) : (
        <div></div>
      )}
    </div>
