...
const defultTabCls = 'cursor-pointer px-[10px] flex items-center gap-2 rounded-[8px] font-bold border border-[#383F47] bg-[##383F47] transition-all duration-300 h-[35px] text-[#FFFFFF99]'
export default function NFTListContent() {
    const { poolList, loading, sortField, sortOrder, setSortField, setSortOrder, onQueryPoolList } = usePoolList({
        pageLimit: 100,
        chain: 'solana',
        tokenStatus: 1,
    })
...

            {
                !loading && poolList.length === 0 && (
                    <Empty className="h-[30vh]">
                        <div className="text-[#BBACA6] text-[14px] flex justify-center items-center">No data</div>
                    </Empty>
                )
            }
